...
\usepackage[scaled=1.13,xcharter,vvarbb]{newtxmath}     % Math font settings (xcharter)
%
% Use Erewhon for text, Source Code Pro for monospace, and Source Sans Pro for sans-serif.
\ifdefined\directlua                                    % If using lualatex, use fontspec to set the fonts
...
\else                                                   % If not using lualatex, use packages to set the fonts
    \usepackage[p,space,sups]{erewhon}
...
\usepackage[font=sf,bf,justification=centering]{caption}% Caption typesetting (sans fonts)
...
\fancypagestyle{fancy}{
    ...
    \fancyhead[LE]{\MakeUppercase{\rightmark\hfill\leftmark}}
    \fancyhead[RO]{\MakeUppercase{\leftmark\hfill\rightmark}}
